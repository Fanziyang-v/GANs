...
def train(options: argparse.Namespace) -> None:
    ...
    C, _, _ = (1, 28, 28) if options.dataset in ('MNIST', 'FashionMNIST') else (3, 32, 32)
    ...
    G = Generator(C, LATENT_SIZE).to(device)
    # Running on multiple GPU or CPU.
    D = nn.DataParallel(D)
    G = nn.DataParallel(G)
    # Initialize weights using Gaussian distribution(mean=0, std=0.02)
...
def inference(options: argparse.Namespace) -> None:
    ...
    # Running on multiple GPU or CPU.
    G = nn.DataParallel(G)
    z = torch.rand(BATCH_SIZE, LATENT_SIZE, 1, 1)
    fake_images = G(z)
